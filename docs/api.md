...
!!! info "このページの範囲"
    ...
    ここでは API の使い方と設計を扱う。DNS、TCP/TLS、ソケット、プロセス、カーネルまでの内部経路は [APIの内部：DNSからカーネルまで](api-internals.md) を読む。
...
`fetch` は `404` や `500` でも通信自体は成功として扱います。そのため、`response.ok` を確認しないコードは、失敗レスポンスを正常なデータとして処理するおそれがあります。
...
`401` と `403` は似ていますが、意味は異なります。**本人確認ができない**状態が `401`、**本人ではあるが許可されない**状態が `403` です。
...
利用者に必要なのは URL 一覧だけではありません。各エンドポイントに、少なくとも次を用意します。
...
API は単なる通信手段ではなく、利用者との契約です。名前、入力、出力、失敗時の振る舞いを揃えるほど、利用側と保守側の変更が容易になります。
