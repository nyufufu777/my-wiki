...
!!! tip "補足: Windows SandboxとAppContainerは別物"

    Windows Sandboxは隔離された一時的なWindows環境を起動する。AppContainerは、同じWindows上で動く1つのプロセスの権限を狭める。前者は未知のプログラムを試す用途、後者はアプリ開発時の最小権限に向く。
...
!!! tip "補足: 有効化はSandboxを今すぐ起動する操作ではない"

    このコマンドはWindowsの機能を追加し、再起動後にWindows Sandboxを起動できるようにする。作業中のアプリや既存のPowerShellが隔離されるわけではない。
...
!!! tip "補足: `.wsb` はSandboxの起動設定ファイル"

    `.wsb` は実行ファイルではなくXML形式の設定である。開くたびにWindows Sandboxが新しい一時環境を起動し、この設定に従って共有・通信・ログオン後の処理を組み立てる。
...
!!! tip "補足: テストは境界が働くことを確かめるためのもの"

    成功することだけを確認すると、隔離設定の漏れに気付きにくい。ここでは意図的に「書けない」「つながらない」を観測し、`.wsb`の設定が実際の挙動へ反映されたことを確認する。
...
!!! warning "補足: 共有はSandboxの穴ではなく、意図的な接続点"

    shared folderやクリップボードは作業を便利にする一方で、Sandbox内の情報がホストへ移る経路にもなる。必要な入力だけを渡すなら、共有フォルダを読み取り専用にし、出力はSandboxを閉じる前に内容を確認してから別経路で回収する。
...
!!! tip "補足: SID・token・DACLの役割"

    SIDは主体を表す識別子、access tokenはプロセスが現在持つSIDや特権の集合、DACLは対象ファイルなどが「誰に何を許すか」を記した表である。アクセス時には、tokenの内容とDACLが照合される。
...
!!! tip "補足: Job ObjectはWindows版の資源管理箱"

    Job Objectへプロセスを入れると、その子プロセスもまとめて管理できる。終了時に一括停止したい、メモリやCPUを上限化したい、といった用途を担う。権限を落とすAppContainerとは担当が異なる。
...
!!! tip "補足: 規則はプログラムの絶対パスに結び付く"

    この例では `C:\Sandbox\Tools\untrusted.exe` を対象にしている。同じ内容のファイルを別の場所へコピーして実行した場合は別プログラムとして扱われるため、規則の対象にはならない。
