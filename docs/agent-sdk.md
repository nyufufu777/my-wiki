...
Agent は Tool を直接の権限として持つのではなく、入力・実行・結果・状態更新を制御ループとして扱います。Guardrail と Trace は後から足す装飾ではなく、このループを安全に観測・制限するための部品です。
...
trace には秘密情報をそのまま残しません。入力・出力・トークン・認証ヘッダーをマスキングし、必要な期間だけ保持します。観測性とプライバシーはトレードオフではなく、両方を設計する対象です。
...
Agent SDK は「賢い関数」を呼ぶためのものではありません。判断、実行、状態、責任、観測を分けて、自律的な処理を運用可能にするための枠組みです。
