...
SDK（Software Development Kit）は、あるサービスや機能をアプリへ組み込むために必要な**道具一式**です。単なるライブラリではなく、利用者が実装・テスト・公開できるように複数の部品をまとめたものです。
...
SDK の価値は、HTTP を短く書けることだけではありません。サービスとの契約を、各言語で間違えにくく、観測・更新しやすい形で提供することにあります。
